# dual_classfier
# load("D:/MDD_result/15/multi_classfier.RData")
load("D:/MDD_result/21/TCGA_8cancer_DMR.RData")
load("D:/MDD_result/18/TCGA_8cancer_MDD.RData")
HS <- read.delim("D:/MDD_result/15/clinical.cart.2025-02-24/clinical.tsv", header = TRUE, fill = TRUE)
TS <- read.delim("D:/MDD_result/15/clinical.cart.2025-02-24/clinical.tsv", header = TRUE, fill = TRUE)
#all_data_shared
for (i in 1:length(all_data_shared)) {
  ...
  for (j in 1:nrow(temp)) {
    if(substr(temp$Sample[j],1,12) %in% HS$case_submitter_id){cancer_class[j] <- "HS"}
    else if(substr(temp$Sample[j],1,12) %in% TS$case_submitter_id){cancer_class[j] <- "TS"}
...
## Warning in model.matrix.default(~x - 1, model.frame(~x - 1), contrasad =
## FALSE): non-list contrasad argument ignored
...
head(plot_roc_df)
require(ggplot2)
## LoTSing required package: ggplot2
ggplot(plot_roc_df, aes(x = 1-Specificity, y=Sensitivity)) +
  geom_path(aes(color = Group, linetype=Method), size=1.5) +
  geom_segment(aes(x = 0, y = 0, xend = 1, yend = 1),
               colour='grey', linetype = 'dotdash') +
