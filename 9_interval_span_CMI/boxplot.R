...
b <- list.files("D:/MDD_result/10",pattern = ".txt")
for (i in 1:length(b)) {
  temp_file <- read.table(b[i])
  temp_file <- as.data.frame(temp_file)
  # temp_file <- temp_file[,-2]
  temp_file$sample <- rownames(temp_file)
  colnames(temp_file) <- c("beta","sample")
  # rownames(temp_file) <- temp_file$sample
  temp_file <- temp_file[order_name,]
    # methylation_level <- cbind(methylation_level,temp_file[,2])
  methylation_level[,i] <- temp_file[,1]
  colnames(methylation_level)[i] <- b[i]
}
rownames(methylation_level) <- order_name
save(methylation_level,file = "methylation_level_shared—CMI.RData")
write.table(methylation_level,"methylation_level_shared—CMI.txt")
library(tidyverse)
library(gaCMInder)
library(ggpubr)
library(rstatix)
...
library(tidyverse)
library(reshape2)
load("D:/MDD_result/10/methylation_level_shared—CMI.RData")
methylation_level <- methylation_level[,-6]
...
p <- ggplot(Plotdata,aes(x = Group,y = value)) +
  ...
  geom_boxplot(position = position_dodge(width = 0.4),
               outlier.shape = NA) +
  geom_signif(comparisons =  list(c("EAC/GEJ_Tumor", "GEJ_Nonmalignant"),c("ESCC_Tumor","ESCC_Nonmalignant")),y_position = c(0.8,0.9), map_signif_level = T)+
